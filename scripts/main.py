...
# python main.py -dataset femnist -model cnn -lr 0.025 --num-rounds 300 --clients-per-round 35 --num-epochs 6 --eval-every 5
import argparse
import importlib
import numpy as np
import os
...
def main():
    ...
    for i in range(num_rounds):
        ...
        # print(np.array(server.model).dtype)
        ...
        """if i==9:
            np.save("data/first",np.array(server.model))
            np.save("data/updates",u)
            np.save("data/after",t)"""
        # Update server model
        server.update_model()
...
def print_metrics(metrics, weights, prefix=''):
    ...
    for metric in metric_names:
        ...
        if prefix=="test_" and metric=="accuracy":
            if np.average(ordered_metric, weights=ordered_weights) > 0.80:
                stop=1
